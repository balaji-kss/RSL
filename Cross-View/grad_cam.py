...
def plot_joints(inp_imgs, inp_skels_org, img_num, view_num, main_joints=5, resize_frac=2):
    ...
    for num in range(num_frames):
        ...
        inp_img = np.squeeze(inp_imgs[:, num, :, :, :])
        # print('inp_img bf: ', inp_img.shape)
        # inp_img = inp_img.transpose((1, 2, 0)) #(224, 224, 3)
        # print('inp_img af: ', inp_img.shape)
        inp_img = inp_img.astype('uint8')
        print('min max: ', np.min(inp_img), np.max(inp_img))
...
def vis_att_map_dyn_train(onlyds=False):

    gcmodel = GradCamModel_DYN(onlyds=onlyds).to('cuda:0')
...
if __name__ == '__main__':
    ...
    vis_att_map_dyn_train(onlyds = onlyds)