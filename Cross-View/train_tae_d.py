...
saveModel = modelRoot + dataset + '/dyan_recon_mask_full/'
...
#
pretrained = '/home/balaji/Documents/code/RSL/NewCV/Cross-View/crossViewModel/NUCLA/dyan_recon/100.pth'
...
optimizer = torch.optim.SGD([{'params':filter(lambda x: x.requires_grad, net.transformer_encoder.parameters()), 'lr':1e-3},
                             {'params':filter(lambda x: x.requires_grad, net.sparse_coding.parameters()), 'lr':1e-5}, 
                             {'params':filter(lambda x: x.requires_grad, net.transformer_decoder.parameters()), 'lr':1e-3}], weight_decay=0.001, momentum=0.9)

net.train()

scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[20, 50, 70], gamma=0.333)
...
for epoch in range(0, Epoch+1):
    ...
    for i, sample in enumerate(trainloader):
        ...
        for clip in range(0, input_v2.shape[1]):
            ...
            tdec_out1, dyan_out1, tenc_out1 = net(v1_clip, t1)
            tdec_out2, dyan_out2, tenc_out2 = net(v2_clip, t2)
