...
saveModel = modelRoot + dataset + '/dyn_reproduce_nobc/'
...
# Drr = stateDict['sparseCoding.rr'].cpu().numpy()
...
net = classificationWSparseCode(num_class=num_class, Npole=1*N+1, Drr=Drr, Dtheta=Dtheta, dataType=dataType, dim=2, fistaLam = 0.1, gpu_id=gpu_id).cuda(gpu_id)
...
for epoch in range(0, Epoch+1):
    ...
    for i, sample in enumerate(trainloader):
        ...
        if fusion:
            ...
            loss1 = lam1 * LossCl1 + lam2 * (torch.mean(clipMSE1))
            loss2 = lam1 * LossCl2 + lam2 * (torch.mean(clipMSE2))
    ...
    print('epoch:', epoch, '|loss:', loss_val, '|cls:', np.mean(np.array(lossCls)), '|Bi:', 0.0,
            ...
            for i, sample in enumerate(valloader):
                ...
                for i in range(0, inputSkeleton.shape[1]):
                    ...
                    if fusion:
                        ...
                        label_clip, _, = net(input_clip, t) # DY+BI
